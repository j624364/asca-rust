...
impl SubRule {
        ...
        loop {
            ...
            if !res.is_empty() {
                let start = match res[0] {
                    ...
                    MatchElement::Syllable(s, _) | MatchElement::SyllBound(s, _) => {
                        SegPos::new(s, 0)
                    }
                };
                let end = match *res.last().unwrap() {
                    ...
                    MatchElement::Syllable(s, _) => {
                        SegPos::new(s, word.syllables[s].segments.len() - 1)
                    }
                };
                if !self.match_before_context_and_exception(&word, start)?
                    || !self.match_after_context_and_exception(&word, end, true)?
                {
    ...
    fn match_before_context_and_exception(
        &self,
        word: &Word,
        pos: SegPos,
    ) -> Result<bool, RuleRuntimeError> {
        ...
        let is_cont_match =
            self.match_before_env(&cont_states, &word_rev, &pos.reversed(word), false, true)?;
        let is_expt_match =
            self.match_before_env(&expt_states, &word_rev, &pos.reversed(word), false, false)?;

        Ok(!is_expt_match && is_cont_match)
    }

    fn match_after_context_and_exception(
        &self,
        word: &Word,
        pos: SegPos,
        inc: bool,
    ...
    fn context_match(
        ...
        match &state.kind {
            ...
            ParseElement::Ipa(s, m) => {
                if self.context_match_ipa(s, m, word, *pos, state.position)? {
                    pos.increment(word);
                    Ok(true)
                } else {
                    Ok(false)
            ...
            ParseElement::Matrix(m, v) => {
                self.context_match_matrix(m, v, word, pos, state.position)
            }
            ParseElement::Syllable(s, t, v) => {
                if ins_match_before {
                    Ok(!pos.at_word_start()
                        && self.context_match_syll(s, t, v, word, pos, forwards)?)
                } else {
                    self.context_match_syll(s, t, v, word, pos, forwards)
                }
            }
            ParseElement::Structure(segs, stress, tone, var) => {
                if ins_match_before {
                    Ok(!pos.at_word_start()
                        && self.context_match_structure(
                            segs, stress, tone, var, word, pos, forwards,
                        )?)
                } else {
                    self.context_match_structure(segs, stress, tone, var, word, pos, forwards)
                }
            }
            ParseElement::Variable(vt, mods) => {
                self.context_match_var(vt, mods, word, pos, forwards, state.position)
            }
            ParseElement::Set(s) => self.context_match_set(s, word, pos, forwards),
            ParseElement::Optional(opt_states, min, max) => self.context_match_option(
                states,
                state_index,
                word,
                pos,
            ...
            ParseElement::Ellipsis => {
                self.context_match_ellipsis(states, state_index, word, pos, forwards)
            }

            ParseElement::EmptySet | ParseElement::Metathesis | ParseElement::Environment(_, _) => {
                unreachable!()
            }
        }
    }

    fn context_match_structure(
        &self,
        items: &[Item],
        stress: &[Option<ModKind>; 2],
        tone: &Option<Tone>,
        var: &Option<usize>,
        word: &Word,
        pos: &mut SegPos,
            ...
            match &item.kind {
                ParseElement::Ellipsis => {
                    if i == items.len() - 1 {
                        ...
                        pos.seg_index = 0;
                        break;
                    } else if self.context_match_ellipis_struct(
                        &items,
                        &mut i,
                        word,
                        pos,
                ...
                ParseElement::Ipa(s, mods) => {
                    if self.context_match_ipa(s, mods, word, *pos, item.position)? {
                        pos.increment(word);
                    } else {
                        return Ok(false);
                    }
                ...
                ParseElement::Matrix(mods, var) => {
                    if !self.context_match_matrix(mods, var, word, pos, item.position)? {
                        return Ok(false);
                    }
                }
        ...
        if let Some(v) = var {
            if forwards {
                self.variables.borrow_mut().insert(
                    *v,
                    VarKind::Syllable(word.syllables[cur_syll_index].clone()),
                );
            } else {
                let mut syll = word.syllables[cur_syll_index].clone();
                syll.segments.make_contiguous().reverse();
                self.variables
                    .borrow_mut()
                    .insert(*v, VarKind::Syllable(syll));
            }
        }
    ...
    fn context_match_ellipis_struct(
        &self,
        items: &[Item],
        index: &mut usize,
        word: &Word,
                ...
                match &items[*index].kind {
                    ParseElement::Ellipsis => {
                        if *index == items.len() - 1 {
                            ...
                            .context_match_ellipis_struct(items, index, word, pos, syll_index)?
                        {
                    ...
                    ParseElement::Ipa(s, mods) => {
                        if self.context_match_ipa(s, mods, word, *pos, items[*index].position)? {
                            pos.increment(word);
                        } else {
                            m = false;
                            break;
    ...
    fn context_match_set(
        ...
        for s in set {
            let res = match &s.kind {
                ParseElement::Variable(vt, mods) => {
                    self.context_match_var(vt, mods, word, pos, forwards, s.position)
                }
                ParseElement::Ipa(seg, mods) => {
                    if self.context_match_ipa(seg, mods, word, *pos, s.position)? {
                        pos.increment(word);
                        Ok(true)
                    } else {
                        Ok(false)
                ...
                ParseElement::Matrix(mods, var) => {
                    self.context_match_matrix(mods, var, word, pos, s.position)
                }
                ParseElement::Syllable(stress, tone, var) => {
                    self.context_match_syll(stress, tone, var, word, pos, forwards)
                }
                ParseElement::WordBound => Ok(word.out_of_bounds(*pos)),
    ...
    fn context_match_var(
        ...
        if let Some(var) = self
            .variables
            .borrow_mut()
            .get(&vt.value.parse::<usize>().unwrap())
        {
            match var {
                VarKind::Segment(s) => {
                    if self.context_match_ipa(s, mods, word, *pos, err_pos)? {
                        pos.increment(word);
                        Ok(true)
                    } else {
                        Ok(false)
    ...
    fn context_match_syll_var(
        &self,
        syll_to_match: &Syllable,
        mods: &Option<Modifiers>,
        word: &Word,
        pos: &mut SegPos,
        forwards: bool,
        ...
        } else if cur_syll.segments != segs_to_match
            || cur_syll.stress != syll_to_match.stress
            || cur_syll.tone != syll_to_match.tone
        {
            return Ok(false);
        }
        pos.syll_index += 1;
    ...
    fn context_match_syll(
        ...
        if let Some(v) = var {
            if forwards {
                self.variables.borrow_mut().insert(
                    *v,
                    VarKind::Syllable(word.syllables[pos.syll_index].clone()),
                );
            } else {
    ...
    fn context_match_matrix(
        ...
        if self.match_modifiers(mods, word, pos, err_pos)? {
            if let Some(v) = var {
                self.variables
                    .borrow_mut()
                    .insert(*v, VarKind::Segment(word.get_seg_at(*pos).unwrap()));
            }
    ...
    fn transform(
        &self,
        word: &Word,
        input: Vec<MatchElement>,
        next_pos: &mut Option<SegPos>,
    ) -> Result<Word, RuleRuntimeError> {
        match self.rule_type {
                    ...
                    match (input[z], input[input.len() - 1 - z]) {
                        ...
                        (MatchElement::Segment(si, _), MatchElement::SyllBound(bi, _)) => {
                            ...
                            if bi < res_word.syllables.len() {
                                ...
                                res_word.syllables[si.syll_index]
                                    .segments
                                    .remove(si.seg_index); // pop_back()
                            } else {
                                res_word.syllables.push(Syllable {
                                    segments: VecDeque::new(),
                                    stress: StressKind::Unstressed,
                                    tone: 0,
                                });
                                res_word
                                    .syllables
                        ...
                        (MatchElement::SyllBound(bi, _), MatchElement::Segment(si, _)) => {
                            ...
                            if bi > 0 {
                                ...
                                res_word.syllables[si.syll_index]
                                    .segments
                                    .remove(si.seg_index); // pop_front()
                            } else {
                                res_word.syllables[si.syll_index]
                                    .segments
                                    .remove(si.seg_index); // pop_front()
                                res_word.syllables.insert(
                                    0,
                                    Syllable {
                                        segments: VecDeque::new(),
                                        stress: StressKind::Unstressed,
                        ...
                        | (MatchElement::Syllable(..), MatchElement::Segment(..)) => {
                            ...
                            return Err(RuleRuntimeError::MetathSyllSegment(
                                self.input[z].position,
                                self.input[end].position,
                            ));
                        }
                        (MatchElement::Syllable(..), MatchElement::SyllBound(..))
                        | (MatchElement::SyllBound(..), MatchElement::Syllable(..)) => {
                            let end = input.len() - 1 - z;
                            return Err(RuleRuntimeError::MetathSyllBoundary(
                                self.input[z].position,
                                self.input[end].position,
                            ));
                        }
                    ...
                    match z {
                        MatchElement::Segment(i, _) => {
                            ...
                            if res_word.syllables.len() <= 1
                                && word.syllables[i.syll_index].segments.len() <= 1
                            {
                                return Err(RuleRuntimeError::DeletionOnlySeg);
                            }
                            res_word.syllables[i.syll_index]
                                .segments
                                .remove(i.seg_index);
                        ...
                        MatchElement::SyllBound(i, _) => {
                            ...
                            res_word.syllables[i - 1].stress = match (
                                res_word.syllables[i - 1].stress,
                                res_word.syllables[i].stress,
                            ) {
                                (StressKind::Primary, _) | (_, StressKind::Primary) => {
                                    StressKind::Primary
                                }
                                (StressKind::Secondary, StressKind::Secondary)
                                | (StressKind::Secondary, StressKind::Unstressed)
                                | (StressKind::Unstressed, StressKind::Secondary) => {
                                    StressKind::Secondary
                                }
                                (StressKind::Unstressed, StressKind::Unstressed) => {
                                    StressKind::Unstressed
                                }
                            };

                            res_word.syllables[i - 1].tone = Self::concat_tone(
                                res_word.syllables[i - 1].tone,
                                res_word.syllables[i].tone,
                            );
            ...
            RuleType::Insertion => {
                ...
                if before_cont.is_empty()
                    && after_cont.is_empty()
                    && before_expt.is_empty()
                    && after_expt.is_empty()
                {
                    return Err(RuleRuntimeError::InsertionNoEnv(
                        self.output.last().unwrap().position,
                    ));
                }
    ...
    fn insertion_match_exceptions(
        ...
        match (before_expt.is_empty(), after_expt.is_empty()) {
            ...
            (false, true) => {
                ...
                let match_bef =
                    self.match_before_env(&before_expt, word_rev, &pos_rev, false, false)?;
                Ok(match_bef)
            }
            (true, false) => {
                // _#
                ...
                if after_expt.len() == 1
                    && after_expt[0].kind == ParseElement::WordBound
                    && !ins_pos.at_word_end(word)
                {
                    return Ok(false);
                }
                let match_aft =
                    self.match_after_env(after_expt, word, &ins_pos, true, false, false)?;
                Ok(match_aft)
            }
            // #_#
            (false, false) => {
                ...
                let match_bef =
                    self.match_before_env(&before_expt, word_rev, &pos_rev, false, false)?;
                let match_aft =
                    self.match_after_env(after_expt, word, &ins_pos, false, false, false)?;
    ...
    fn insertion_match(
        ...
        word: &Word,
        start_pos: SegPos,
    ) -> Result<Option<SegPos>, RuleRuntimeError> {
        let empty = Vec::new();
        let (before_cont, after_cont) = self.get_context().unwrap_or((&empty, &empty));
        let (before_expt, after_expt) = self.get_exceptions().unwrap_or((&empty, &empty));
        ...
        'outer: while word.in_bounds(start_pos) {
            match self.insertion_after(bef_states, word, start_pos)? {
                Some(mut ins_pos) => {
                    ...
                    if let ParseElement::SyllBound | ParseElement::Structure(..) =
                        aft_states[0].kind
                    {
    ...
    fn insertion_before(
        ...
        while word.in_bounds(cur_pos) {
            ...
            if self.context_match(states, &mut state_index, word, &mut cur_pos, true, true)? {
                if match_begin.is_none() {
                    ...
                    if let ParseElement::Syllable(..) | ParseElement::SyllBound =
                        states.first().unwrap().kind
                    {
                        sp.decrement(word);
                        sp.seg_index += 1;
                    }
        ...
        if match_begin.is_none() {
            if let ParseElement::WordBound | ParseElement::SyllBound | ParseElement::Structure(..) =
                states.first().unwrap().kind
            {
                let sy = word.syllables.len() - 1;
                let sg = word.syllables[sy].segments.len();
    ...
    fn insert(
        ...
        word: &Word,
        pos: SegPos,
        is_context_after: bool,
    ) -> Result<(Word, Option<SegPos>), RuleRuntimeError> {
        let mut res_word = word.clone();
        let mut pos = pos;
            ...
            match &state.kind {
                ParseElement::Ipa(seg, mods) => {
                    if let Some(syll) = res_word.syllables.get_mut(pos.syll_index) {
                        ...
                        if let Some(m) = mods {
                            let lc =
                                res_word.apply_seg_mods(&self.alphas, m, pos, state.position)?;
                            if lc > 0 {
                                pos.seg_index += lc.unsigned_abs() as usize;
                            }
                        }
                ...
                ParseElement::SyllBound => {
                    ...
                    while first_syll.segments.len() > pos.seg_index {
                        second_syll
                            .segments
                            .push_front(first_syll.segments.pop_back().unwrap());
                    }
                ...
                ParseElement::Syllable(stress, tone, var) => {
                    if pos.at_syll_start() {
                        ...
                        if let Some(syll) = res_word.syllables.get_mut(pos.syll_index) {
                            ...
                            if let Some(v) = var {
                                self.variables.borrow_mut().insert(
                                    *v,
                                    VarKind::Syllable(res_word.syllables[pos.syll_index].clone()),
                                );
                            }
                            continue;
                        } else {
                    ...
                    let syll = res_word
                        .syllables
                        .get_mut(pos.syll_index)
                        .expect("pos should not be out of bounds");

                    while syll.segments.len() > pos.seg_index {
                        new_syll
                            .segments
                            .push_front(syll.segments.pop_back().unwrap());
                    }
                    ...
                    if let Some(v) = var {
                        self.variables.borrow_mut().insert(
                            *v,
                            VarKind::Syllable(res_word.syllables[pos.syll_index - 1].clone()),
                        );
                    }
                }
                ParseElement::Structure(items, stress, tone, var) => {
                    let insert_syll =
                        self.gen_syll_from_struct(items, stress, tone, var, state.position, true)?;

                    if pos.at_syll_start() {
                        res_word.syllables.insert(pos.syll_index, insert_syll);
                        pos.syll_index += 1;
                        pos.seg_index = 0;
                        continue;
                    }

                    let old_syll = res_word
                        .syllables
                        .get_mut(pos.syll_index)
                        .expect("pos should not be out of bounds");
                    ...
                    while old_syll.segments.len() > pos.seg_index {
                        new_syll
                            .segments
                            .push_front(old_syll.segments.pop_back().unwrap());
                    }
                    ...
                    if !new_syll.segments.is_empty() {
                        res_word
                            .syllables
                            .insert(pos.syll_index + 1 + adjustment, new_syll);
                        ...
                        match var {
                            VarKind::Segment(seg) => {
                                if let Some(syll) = res_word.syllables.get_mut(pos.syll_index) {
                                    ...
                                    res_word
                                        ...
                                        .push_back(*seg);
                                    if let Some(m) = mods {
                                        let lc = res_word.apply_seg_mods(
                                            &self.alphas,
                                            m,
                                            pos,
                                            state.position,
                            ...
                            VarKind::Syllable(syll) => {
                                ...
                                if pos.at_syll_start() {
                                    ...
                                    let before_syll =
                                        res_word.syllables.get_mut(pos.syll_index).unwrap();
                                    let mut after_syll = Syllable::new();
                                    while before_syll.segments.len() > pos.seg_index {
                                        after_syll
                                            .segments
                                            .push_front(before_syll.segments.pop_back().unwrap());
                                    }
                ...
                ParseElement::Matrix(..) => {
                    return Err(RuleRuntimeError::InsertionMatrix(state.position))
                }
                ParseElement::Set(_) => return Err(RuleRuntimeError::LonelySet(state.position)),
                ParseElement::EmptySet
                | ParseElement::Metathesis
                | ParseElement::Ellipsis
                | ParseElement::Optional(..)
                | ParseElement::WordBound
                | ParseElement::Environment(..) => unreachable!(),
            }
        }
    ...
    fn apply_syll_mods(
        ...
        word.syllables
            .get_mut(syll_index)
            .unwrap()
            .apply_syll_mods(&self.alphas, mods, err_pos)?;

        if let Some(v) = var {
            self.variables
                .borrow_mut()
                .insert(*v, VarKind::Syllable(word.syllables[syll_index].clone()));
        }
        Ok(())
    }
    ...
    fn apply_seg_mods(
        ...
        if let Some(v) = var {
            self.variables.borrow_mut().insert(
                *v,
                VarKind::Segment(word.syllables[pos.syll_index].segments[pos.seg_index]),
            );
        }
    ...
    fn gen_syll_from_struct(
        ...
        err_pos: Position,
        is_inserting: bool,
    ) -> Result<Syllable, RuleRuntimeError> {
        ...
        for item in items {
            match &item.kind {
                ParseElement::Ellipsis => {
                    return Err(if is_inserting {
                        RuleRuntimeError::InsertionEllipsis(item.position)
                    } else {
                        RuleRuntimeError::SubstitutionEllipsis(item.position)
                    })
                }
                ParseElement::Matrix(..) => {
                    return Err(if is_inserting {
                        RuleRuntimeError::InsertionMatrix(item.position)
                    } else {
                        RuleRuntimeError::SubstitutionMatrix(item.position)
                    })
                }
                ParseElement::Ipa(mut segment, modifiers) => {
                    let mut len = 1;
                    if let Some(mods) = modifiers {
                        segment.apply_seg_mods(
                            &self.alphas,
                            mods.nodes,
                            mods.feats,
                            item.position,
                        ...
                        len = match mods.suprs.length {
                            [None, None] => 1,
                            [None, Some(v)] => {
                                if v.as_bool(&self.alphas, item.position)? {
                                    3
                                } else {
                                    1
                                }
                            ...
                            [Some(l), None] => {
                                if l.as_bool(&self.alphas, item.position)? {
                                    2
                                } else {
                                    1
                                }
                            ...
                            [Some(l), Some(v)] => match (
                                l.as_bool(&self.alphas, item.position)?,
                                v.as_bool(&self.alphas, item.position)?,
                            ) {
                                (true, true) => 3,
                                (true, false) => 2,
                                (false, false) => 1,
                                (false, true) => {
                                    Err(RuleRuntimeError::OverlongPosLongNeg(item.position))?
                                }
                            },
                        };
                ...
                ParseElement::Variable(num, modifiers) => {
                    if let Some(var) = self.variables.borrow_mut().get(&num.value.parse().unwrap())
                    {
                        match var {
                            VarKind::Syllable(_) => {
                                return Err(RuleRuntimeError::SyllVarInsideStruct(item.position))
                            }
                                ...
                                if let Some(mods) = modifiers {
                                    ...
                                    len = match mods.suprs.length {
                                        ...
                                        [Some(l), Some(v)] => match (
                                            ...
                                            (false, false) => 1,
                                            (false, true) => Err(
                                                RuleRuntimeError::OverlongPosLongNeg(item.position),
                                            )?,
                                        },
                                    };
                                    // TODO: Ignore syll suprs?
        ...
        if let Some(v) = var {
            self.variables
                .borrow_mut()
                .insert(*v, VarKind::Syllable(syll.clone()));
        }
            ...
            match &out_state.kind {
                    ...
                    match input[state_index] {
                        MatchElement::Syllable(sp, _) => {
                            ...
                            let syll = res_word
                                .syllables
                                .get_mut(sp)
                                .expect("SyllPos is validated index");

                            syll.segments = self
                                .gen_syll_from_struct(
                                    items,
                                    stress,
                                    tone,
                                    var,
                        ...
                        MatchElement::Segment(sp, _) => {
                            ...
                            let insert_syll = self.gen_syll_from_struct(
                                items,
                                stress,
                                tone,
                                var,
                            ...
                            if !new_syll.segments.is_empty() {
                                res_word
                                    .syllables
                                    .insert(sp.syll_index + 1 + adjustment, new_syll);
                        ...
                        MatchElement::SyllBound(_, _) => {
                            return Err(RuleRuntimeError::SubstitutionSyllBound(
                                in_state.position,
                                out_state.position,
                            ))
                        }
                ...
                ParseElement::Matrix(m, v) => {
                    ...
                    match input[state_index] {
                        MatchElement::Segment(mut sp, _) => {
                            ...
                            match total_len_change[sp.syll_index].cmp(&0) {
                                std::cmp::Ordering::Greater => {
                                    sp.seg_index +=
                                        total_len_change[sp.syll_index].unsigned_abs() as usize
                                }
                            ...
                            let lc =
                                self.apply_seg_mods(&mut res_word, sp, m, v, out_state.position)?;
                        ...
                        MatchElement::SyllBound(..) => {
                            return Err(RuleRuntimeError::SubstitutionBoundMod(
                                in_state.position,
                                out_state.position,
                            ))
                        }
                ...
                ParseElement::Ipa(seg, mods) => match input[state_index] {
                    MatchElement::Segment(mut sp, _) => {
                        ...
                        let lc = res_word.syllables[sp.syll_index].replace_segment(
                            sp.seg_index,
                            seg,
                            mods,
                            &self.alphas,
                    ...
                    MatchElement::Syllable(..) | MatchElement::SyllBound(..) => {
                        return Err(RuleRuntimeError::SubstitutionSylltoMatrix(
                            in_state.position,
                            out_state.position,
                        ))
                    }
                    ...
                    if let Some(var) = self.variables.borrow_mut().get(&num.value.parse().unwrap())
                    {
                        match (input[state_index], var) {
                            ...
                            (MatchElement::Syllable(sp, _), VarKind::Syllable(syll)) => {
                                ...
                                last_pos.seg_index = 0;
                                if let Some(m) = mods {
                                    res_word.syllables[sp].apply_syll_mods(
                                        &self.alphas,
                                        &m.suprs,
                                        num.position,
                                    )?;
                            ...
                            (MatchElement::Segment(sp, _), VarKind::Syllable(insert_syll)) => {
                                ...
                                if old_syll.segments.is_empty() {
                                    *old_syll = insert_syll.clone();
                                    ...
                                    res_word
                                        .syllables
                                        .insert(sp.syll_index + 1, insert_syll.clone());
                                    adjustment = 1;
                                }
                                if !new_syll.segments.is_empty() {
                                    res_word
                            ...
                            | (MatchElement::Syllable(..), VarKind::Segment(..)) => {
                                return Err(RuleRuntimeError::SubstitutionSylltoSeg(
                                    in_state.position,
                                    out_state.position,
                                ))
                            }
                            (MatchElement::SyllBound(..), VarKind::Syllable(_)) => {
                                return Err(RuleRuntimeError::SubstitutionSyllBound(
                                    in_state.position,
                                    out_state.position,
                                ))
                ...
                ParseElement::Set(set_output) => {
                    ...
                    match &in_state.kind {
                        ParseElement::Set(set_input) => {
                            if set_input.len() == set_output.len() {
                                match input[state_index] {
                                    MatchElement::Segment(mut sp, set_index) => {
                                        let Some(i) = set_index else {
                                            return Err(RuleRuntimeError::LonelySet(
                                                in_state.position,
                                            ));
                                        };
                                        match total_len_change[sp.syll_index].cmp(&0) {
                                            std::cmp::Ordering::Greater => {
                                                sp.seg_index += total_len_change[sp.syll_index]
                                                    .unsigned_abs()
                                                    as usize
                                            }
                                            std::cmp::Ordering::Less => {
                                        ...
                                        match &set_output[i].kind {
                                            ParseElement::Ipa(seg, mods) => {
                                                res_word.syllables[sp.syll_index].segments
                                                    [sp.seg_index] = *seg;
                                                if let Some(m) = mods {
                                                    let lc = res_word.apply_seg_mods(
                                                        &self.alphas,
                                            ...
                                            ParseElement::Variable(num, mods) => {
                                                if let Some(var) = self
                                                    .variables
                                                    .borrow_mut()
                                                    .get(&num.value.parse().unwrap())
                                                {
                                                    match var {
                                                        VarKind::Segment(seg) => {
                                                            res_word.syllables[sp.syll_index]
                                                                .segments[sp.seg_index] = *seg;
                                                            if let Some(m) = mods {
                                                                let lc = res_word.apply_seg_mods(
                                                                    &self.alphas,
                                                                    m,
                                                                ...
                                                                total_len_change[sp.syll_index] +=
                                                                    lc;
                                                                if lc > 0 {
                                                                    last_pos.seg_index +=
                                                                        lc.unsigned_abs() as usize;
                                                        ...
                                                        VarKind::Syllable(insert_syll) => {
                                                            ...
                                                            while old_syll.segments.len()
                                                                > sp.seg_index
                                                            {
                                                                new_syll.segments.push_front(
                                                                    old_syll
                                                                        .segments
                                                                        .pop_back()
                                                                        .unwrap(),
                                                            ...
                                                            if !new_syll.segments.is_empty() {
                                                                res_word.syllables.insert(
                                                                    sp.syll_index + 1 + adjustment,
                                                                    new_syll,
                                                                );
                                                                last_pos.syll_index =
                                                                    sp.syll_index + 2 + adjustment;
                                                            } else {
                                                                last_pos.syll_index =
                                                                    sp.syll_index + 1 + adjustment;
                                                            }
                                                    ...
                                                    return Err(RuleRuntimeError::UnknownVariable(
                                                        num.clone(),
                                                    ));
                                                }
                                            }
                                            ...
                                            | ParseElement::Syllable(..) => {
                                                return Err(
                                                    RuleRuntimeError::SubstitutionSylltoSeg(
                                                        in_state.position,
                                                        set_output[i].position,
                                                    ),
                                                )
                                            ...
                                            ParseElement::WordBound => {
                                                return Err(RuleRuntimeError::WordBoundSetLocError(
                                                    set_output[i].position,
                                                ))
                                            }
                                            _ => unreachable!(),
                                    ...
                                    MatchElement::Syllable(sp, set_index) => {
                                        ...
                                        match &set_output[i].kind {
                                            ...
                                            ParseElement::Syllable(stress, tone, var) => {
                                                let sups =
                                                    SupraSegs::from(*stress, [None, None], *tone);
                                                self.apply_syll_mods(
                                                    &mut res_word,
                                                    sp,
                                                    &sups,
                                            ...
                                            ParseElement::Ipa(..) => {
                                                return Err(
                                                    RuleRuntimeError::SubstitutionSylltoMatrix(
                                                        in_state.position,
                                                        set_output[i].position,
                                                    ),
                                                )
                                            ...
                                            ParseElement::SyllBound => {
                                                return Err(
                                                    RuleRuntimeError::SubstitutionSylltoBound(
                                                        in_state.position,
                                                        set_output[i].position,
                                                    ),
                                                )
                                    ...
                                    MatchElement::SyllBound(sp, set_index) => {
                                        ...
                                        match &set_output[i].kind {
                                            ...
                                            _ => {
                                                return Err(
                                                    RuleRuntimeError::SubstitutionSyllBound(
                                                        in_state.position,
                                                        out_state.position,
                                                    ),
                                                )
                                ...
                                return Err(RuleRuntimeError::UnevenSet(
                                    in_state.position,
                                    out_state.position,
                                ));
                            }
                ...
                match &z.kind {
                    ParseElement::Ipa(seg, mods) => {
                        if let Some(syll) = res_word.syllables.get_mut(pos.syll_index) {
                            ...
                            if let Some(m) = mods {
                                let lc =
                                    res_word.apply_seg_mods(&self.alphas, m, pos, z.position)?;
                                if lc > 0 {
                                    pos.seg_index += lc.unsigned_abs() as usize;
                                }
                            }
                            ...
                            if let Some(syll) = res_word.syllables.get_mut(pos.syll_index) {
                                ...
                                if let Some(v) = var {
                                    self.variables.borrow_mut().insert(
                                        *v,
                                        VarKind::Syllable(
                                            res_word.syllables[pos.syll_index].clone(),
                                        ),
                                    );
                                }
                                continue;
                    ...
                    ParseElement::Structure(items, stress, tone, var) => {
                        let insert_syll =
                            self.gen_syll_from_struct(items, stress, tone, var, z.position, true)?;

                        if let Some(v) = var {
                            self.variables
                                .borrow_mut()
                                .insert(*v, VarKind::Syllable(insert_syll.clone()));
                        }
                        ...
                        if old_syll.segments.is_empty() {
                            *old_syll = insert_syll.clone();
                            ...
                            res_word
                                .syllables
                                .insert(pos.syll_index + 1, insert_syll.clone());
                            adjustment = 1;
                        }
                        if !new_syll.segments.is_empty() {
                            res_word
                            ...
                            match var {
                                VarKind::Segment(seg) => {
                                    if res_word.in_bounds(pos) {
                                        res_word.syllables[pos.syll_index]
                                            .segments
                                            .insert(pos.seg_index, *seg);
                                    } else if let Some(syll) =
                                        res_word.syllables.get_mut(pos.syll_index)
                                    {
                                ...
                                VarKind::Syllable(syll) => {
                                    ...
                                    if pos.at_syll_start() {
                                        ...
                                        let mut after_syll = Syllable::new();
                                        while before_syll.segments.len() > pos.seg_index {
                                            after_syll.segments.push_front(
                                                before_syll.segments.pop_back().unwrap(),
                                            );
                                        }
                                        res_word.syllables.insert(pos.syll_index + 1, after_syll);
                                        res_word.syllables.insert(pos.syll_index + 1, new_syll);
                    ...
                    ParseElement::Matrix(..) => {
                        return Err(RuleRuntimeError::InsertionMatrix(z.position))
                    }
            ...
            for &z in input.iter().skip(start_index).rev() {
                match z {
                    MatchElement::Segment(mut sp, _) => {
                        ...
                        // remove segment
                        if res_word.syllables.len() <= 1
                            && word.syllables[sp.syll_index].segments.len() <= 1
                        {
                            return Err(RuleRuntimeError::DeletionOnlySeg);
                        }
                        res_word.syllables[sp.syll_index]
    ...
    fn input_match_at(
        ...
        word: &Word,
        start_index: SegPos,
    ) -> Result<(Vec<MatchElement>, Option<SegPos>), RuleRuntimeError> {
            ...
            if self.input_match_item(
                ...
                if match_begin.is_none() {
                    ...
                    match captures.last().expect("") {
                        ...
                        MatchElement::Syllable(sp, _) | MatchElement::SyllBound(sp, _) => {
                            match_begin = Some(SegPos {
                                syll_index: *sp,
                                seg_index: 0,
                            })
                        }
    ...
    fn input_match_item(
        ...
        match &states[*state_index].kind {
            ParseElement::Variable(vt, m) => {
                if self.input_match_var(captures, state_index, vt, m, word, seg_pos, err_pos)? {
                    *state_index += 1;
                    Ok(true)
                } else {
                    Ok(false)
            ...
            ParseElement::Ipa(s, m) => {
                if self.input_match_ipa(captures, s, m, word, seg_pos, err_pos)? {
            ...
            ParseElement::Matrix(m, v) => {
                if self.input_match_matrix(captures, m, v, word, seg_pos, err_pos)? {
                    seg_pos.increment(word);
                    *state_index += 1;
                    Ok(true)
                } else {
            ...
            ParseElement::Set(s) => {
                if self.input_match_set(captures, state_index, s, word, seg_pos)? {
                    *state_index += 1;
                    Ok(true)
                } else {
                    Ok(false)
            ...
            ParseElement::SyllBound => {
                if self.input_match_syll_bound(captures, *seg_pos) {
            ...
            ParseElement::Syllable(s, t, v) => {
                self.input_match_syll(captures, state_index, s, t, v, word, seg_pos)
            }
            ParseElement::Structure(segs, stress, tone, var) => self.input_match_structure(
                captures,
                state_index,
                segs,
                stress,
            ...
            ParseElement::Ellipsis => {
                self.input_match_ellipsis(captures, word, seg_pos, states, state_index)
            }

            ParseElement::Optional(..)
            | ParseElement::Environment(_, _)
            | ParseElement::EmptySet
            | ParseElement::WordBound
            | ParseElement::Metathesis => unreachable!(),
        }
    }

    fn input_match_structure(
        &self,
        captures: &mut Vec<MatchElement>,
        state_index: &mut usize,
        items: &[Item],
        ...
        for (mut i, item) in items.iter().enumerate() {
            ...
            match &item.kind {
                ...
                ParseElement::Variable(num, mods) => match self
                    .variables
                    .borrow_mut()
                    .get(&num.value.parse::<usize>().unwrap())
                {
                    Some(var) => match var {
                        VarKind::Segment(s) => {
                            if self.context_match_ipa(s, mods, word, *pos, item.position)? {
    ...
    fn concat_tone(prev: Tone, aft: Tone) -> Tone {
        ...
        if nums.len() > 4 {
            ...
            arr[1] = if nums.len() == 5 && nums[1] == nums[3] {
                // 1 2541 2
                nums[2]
            } else {
    ...
    fn match_stress(
        ...
        if let Some(str) = stress[0] {
            match str {
                ModKind::Binary(bm) => match bm {
                    BinMod::Negative => {
                        if syll.stress != StressKind::Unstressed {
                            return Ok(false);
                        }
                    }
                    BinMod::Positive => {
                ...
                ModKind::Alpha(am) => match am {
                    AlphaMod::Alpha(ch) => {
                        ...
                        if let Some(alph) = self.alphas.borrow().get(&ch) {
                            ...
                            match syll.stress {
                                StressKind::Primary | StressKind::Secondary => {
                                    if !pos {
                                        return Ok(false);
                                    }
                                }
        ...
        if let Some(str) = stress[1] {
            match str {
                ModKind::Binary(bm) => match bm {
                    BinMod::Negative => {
                        if syll.stress == StressKind::Secondary {
                            return Ok(false);
                        }
                    }
                    BinMod::Positive => {
                    ...
                    AlphaMod::Alpha(ch) => {
                        ...
                        if let Some(alph) = self.alphas.borrow().get(&ch) {
                            ...
                            match syll.stress {
                                ...
                                StressKind::Primary | StressKind::Unstressed => {
                                    if pos {
                                        return Ok(false);
                                    }
                                }
    ...
    fn input_match_set(
        ...
        for (i, s) in set.iter().enumerate() {
            let res = match &s.kind {
                ParseElement::Variable(vt, mods) => {
                    self.input_match_var(captures, state_index, vt, mods, word, pos, s.position)
                }
                ParseElement::Ipa(seg, mods) => {
                    if self.input_match_ipa(captures, seg, mods, word, pos, s.position)? {
                        pos.increment(word);
                        Ok(true)
                    } else {
                        Ok(false)
                ...
                ParseElement::Matrix(mods, var) => {
                    if self.input_match_matrix(captures, mods, var, word, pos, s.position)? {
                        pos.increment(word);
                        Ok(true)
                    } else {
                        Ok(false)
                ...
                ParseElement::Syllable(stress, tone, var) => {
                    self.input_match_syll(captures, state_index, stress, tone, var, word, pos)
                }
                ParseElement::SyllBound => {
                    if pos.at_syll_start() {
    ...
    fn input_match_var(
            ...
            .get(&vt.value.parse::<usize>().unwrap())
        {
            Some(var) => match var {
                VarKind::Segment(s) => {
                    if self.input_match_ipa(captures, s, mods, word, pos, err_pos)? {
                        pos.increment(word);
                        Ok(true)
                    } else {
                        Ok(false)
                ...
                VarKind::Syllable(s) => {
                    self.input_match_syll_var(captures, state_index, s, mods, word, pos)
                }
            },
    ...
    fn match_ipa_with_modifiers(
        ...
        for (i, n) in mods.nodes.iter().enumerate() {
            if n.is_some() {
                joined_mods.nodes[i] = *n
            }
        }
        for (i, f) in mods.feats.iter().enumerate() {
            if f.is_some() {
                joined_mods.feats[i] = *f
            }
        }
    ...
    fn match_modifiers(
        ...
        let seg = word
            .get_seg_at(*pos)
            .expect("Segment Position should be within bounds");
    ...
    fn match_supr_mod_seg(
        ...
        if !self.match_stress(&mods.stress, syll)? {
            return Ok(false);
        }
        if !self.match_seg_length(word, &mods.length, pos)? {
            return Ok(false);
        }
                ...
                ModKind::Alpha(am) => match am {
                    AlphaMod::Alpha(ch) => {
                        ...
                        if !alpha_assigned {
                            self.alphas
                                .borrow_mut()
                                .insert(ch, Alpha::Supra(seg_length > 1));
                        }
                    }
                    AlphaMod::InvAlpha(ch) => {
                        let mut alpha_assigned = false; // needed because of borrow checker weirdness. See: https://github.com/rust-lang/rust/issues/113792
                ...
                ModKind::Alpha(am) => match am {
                    AlphaMod::Alpha(ch) => {
                        ...
                        if !alpha_assigned {
                            self.alphas
                                .borrow_mut()
                                .insert(ch, Alpha::Supra(seg_length > 2));
                        }
                    }
                    AlphaMod::InvAlpha(ch) => {
                        let mut alpha_assigned = false; // needed because of borrow checker weirdness. See: https://github.com/rust-lang/rust/issues/113792
                ...
                AlphaMod::Alpha(ch) => {
                    if let Some(alph) = self.alphas.borrow().get(ch) {
                        if let Some((n, m)) = alph.as_node() {
                            ...
                            return Ok(seg.node_match(NodeKind::Labial, place.lab)
                    ...
                    if node == NodeKind::Place {
                        ...
                        self.alphas
                            .borrow_mut()
                            .insert(*ch, Alpha::Node(node, seg.get_node(node)));
                    }
                    Ok(true)
                }
                AlphaMod::InvAlpha(inv_ch) => {
                    if let Some(alph) = self.alphas.borrow().get(inv_ch) {
                        if let Some((n, m)) = alph.as_node() {
                            ...
                            Ok(!seg.node_match(NodeKind::Labial, place.lab)
                                || !seg.node_match(NodeKind::Coronal, place.cor)
                                || !seg.node_match(NodeKind::Dorsal, place.dor)
                                || !seg.node_match(NodeKind::Pharyngeal, place.phr))
                        } else {
            ...
            ModKind::Alpha(am) => match am {
                ...
                AlphaMod::InvAlpha(inv_ch) => {
                    ...
                    if let Some(f) = seg.get_feat(node, mask) {
                        self.alphas
                            .borrow_mut()
                            .insert(*inv_ch, Alpha::Feature(f == 0));
                        Ok(true)
                    } else {
                        // Maybe err?
                        Ok(false)
