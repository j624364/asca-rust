...
impl fmt::Display for SyntaxError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::AlreadyInitialisedVariable(set_item, new_item, num) => write!(f, "Variable '{}' is already initialised as {}", num, set_item.kind),
            Self::BadVariableAssignment(it) => write!(f, "A variable can only be assigned to a primative (C, V, etc.) or a matrix"),
