use crate::{
    error::AliasSyntaxError, FType, FeatType, NodeType, SupraType, CARDINALS_TRIE, DIACRITS,
};
...
impl<'a> AliasLexer<'a> {
    ...
    fn get_bracket(&mut self) -> Result<Option<AliasToken>, AliasSyntaxError> {
        ...
        match self.curr_char() {
            ']' => {
                tokenkind = AliasTokenKind::RightSquare;
                value = "]".to_string();
                self.inside_matrix = false
            }
            '[' => {
                if self.inside_matrix {
                    return Err(AliasSyntaxError::NestedBrackets(
                        self.kind, self.line, start,
                    ));
                }
                tokenkind = AliasTokenKind::LeftSquare;
                value = "[".to_string();
        ...
        Ok(Some(AliasToken::new(
            tokenkind,
            value,
            AliasPosition::new(self.kind, self.line, start, self.pos),
        )))
    }
    ...
    fn feature_match(&mut self, buffer: &str) -> Option<AliasTokenKind> {
        ...
        match buffer.to_lowercase().as_str() {
            ...
            | "dr" | "delay" | "drelease" | "del.rel" | "drel" => {
                Some(Feature(Feat(DelayedRelease)))
            }
            ...
            "spreadglottis" | "spreadglot" | "spread" | "s.g." | "s.g" | "sg" => {
                Some(Feature(Feat(SpreadGlottis)))
            }
            ...
            | "ldent" | "ldl" => Some(Feature(Feat(Labiodental))),
    ...
    fn get_feature(&mut self) -> Result<Option<AliasToken>, AliasSyntaxError> {
        ...
        if buffer.len() <= 1 {
            return Err(AliasSyntaxError::ExpectedAlphabetic(
                self.curr_char(),
                self.kind,
                self.line,
                self.pos,
        ...
        let Some(tkn_kind) = self.feature_match(&buffer) else {
            return Err(AliasSyntaxError::UnknownFeature(
                buffer,
                AliasPosition::new(self.kind, self.line, start, self.pos),
            ));
        };

        if let AliasTokenKind::Feature(FeatType::Supr(SupraType::Tone)) = tkn_kind {
            if mod_val == "+" || mod_val == "-" {
    ...
    fn get_special_char(&mut self) -> Result<Option<AliasToken>, AliasSyntaxError> {
        ...
        let value = match self.curr_char() {
            ',' => {
                tokenkind = AliasTokenKind::Comma;
                self.chop(1)
            }
            ':' => {
                tokenkind = AliasTokenKind::Colon;
                self.chop(1)
            }
            '#' => {
                tokenkind = AliasTokenKind::WordBoundary;
                self.chop(1)
            }
            '$' => {
                tokenkind = AliasTokenKind::SyllBoundary;
                self.chop(1)
            }
            '%' => {
                tokenkind = AliasTokenKind::Syllable;
                self.chop(1)
            }
            '*' => {
                tokenkind = AliasTokenKind::Star;
                self.chop(1)
            }
            '∅' => {
                tokenkind = AliasTokenKind::EmptySet;
                self.chop(1)
            }
            '_' => {
                tokenkind = AliasTokenKind::Underline;
                self.chop(1)
            }
            '<' => {
            ...
            '=' => match self.next_char() {
                ...
                _ => {
                    tokenkind = AliasTokenKind::Equals;
                    self.chop(1)
                }
            },
            '-' => match self.next_char() {
                ...
                _ => {
                    return Err(AliasSyntaxError::ExpectedCharArrow(
                        self.next_char(),
                        self.kind,
                        self.line,
                        self.pos,
    ...
    fn get_diacritic(&mut self) -> Option<AliasToken> {
        ...
        for (i, d) in DIACRITS.iter().enumerate() {
            if char == d.diacrit {
                self.advance();
                return Some(AliasToken::new(
                    AliasTokenKind::Diacritic(i as u8),
                    char.to_string(),
                    AliasPosition::new(self.kind, self.line, start, self.pos),
                ));
            }
    ...
    fn get_enby(&mut self) -> Result<Option<AliasToken>, AliasSyntaxError> {
        ...
        if !self.inside_matrix {
            return Err(AliasSyntaxError::OutsideBrackets(
                self.kind, self.line, self.pos,
            ));
        }
        ...
        match self.curr_char() {
            ':' => self.advance(),
            _ => {
                return Err(AliasSyntaxError::ExpectedCharColon(
                    self.curr_char(),
                    self.kind,
                    self.line,
                    self.pos,
        ...
        match self.get_numeric() {
            Some(num) => Ok(Some(AliasToken::new(
                tkn_kind,
                num.value,
                AliasPosition::new(self.kind, self.line, start, self.pos),
            ))),
            _ => Err(AliasSyntaxError::ExpectedNumber(
                self.curr_char(),
                self.kind,
                self.line,
                self.pos,
    ...
    fn string_match(
        ...
        buffer: String,
        start: usize,
    ) -> Result<AliasTokenKind, AliasSyntaxError> {
        use AliasTokenKind::*;
        use FeatType::*;
        use SupraType::*;
        match buffer.to_lowercase().as_str() {
            "tone" | "ton" | "tne" | "tn" => Ok(Feature(Supr(Tone))),
            _ => Err(AliasSyntaxError::UnknownEnbyFeature(
                buffer.clone(),
                AliasPosition::new(self.kind, self.line, start, start + buffer.len()),
            )),
        }
    }
    ...
    fn get_unicode_escape(&mut self) -> Result<char, AliasSyntaxError> {
        match self.curr_char() {
            ...
            _ => Err(AliasSyntaxError::UnknownEscapeChar(
                self.curr_char(),
                self.kind,
                self.line,
                self.pos,
    ...
    fn parse_unicode_escape(&mut self) -> Result<char, AliasSyntaxError> {
        if self.curr_char() != '{' {
            return Err(AliasSyntaxError::ExpectedLeftCurly(
                self.curr_char(),
                self.kind,
                self.line,
                self.pos,
        ...
        self.trim_whitespace();
        if self.curr_char() != '}' {
            return Err(AliasSyntaxError::ExpectedRightCurly(
                self.curr_char(),
                self.kind,
                self.line,
                self.pos,
        ...
        let Ok(val) = u32::from_str_radix(&buf, 16) else {
            return Err(AliasSyntaxError::InvalidUnicodeEscape(
                buf, self.kind, self.line, err_pos,
            ));
        };
        // Conv u32 to char
    ...
    fn get_named_escape(&mut self) -> Result<char, AliasSyntaxError> {
        ...
        if let Ok(esc) = NamedEscape::try_from(buf.as_ref()) {
            ...
            Err(AliasSyntaxError::InvalidNamedEscape(
                buf, self.kind, self.line, err_pos,
            ))
        }
    }
    ...
    fn get_unicode_string(&mut self) -> Result<Option<AliasToken>, AliasSyntaxError> {
        ...
        Ok(Some(AliasToken::new(
            AliasTokenKind::String,
            buffer,
            AliasPosition::new(self.kind, self.line, start, end),
        )))
    }
    ...
    fn get_next_token(&mut self) -> Result<AliasToken, AliasSyntaxError> {
        ...
        if !self.has_more_chars() {
            return Ok(AliasToken::new(
                AliasTokenKind::Eol,
                String::new(),
                AliasPosition::new(self.kind, self.line, self.pos, self.pos + 1),
            ));
        }
        ...
        match self.kind {
            AliasKind::Deromaniser => {
                if self.past_arrow {
                    ...
                    if let Some(str_token) = self.get_enby()? {
                        return Ok(str_token);
                    }
                } else {
                    if let Some(plu_token) = self.get_repl_plus() {
                        return Ok(plu_token);
                    }
                    if let Some(str_token) = self.get_unicode_string()? {
                        return Ok(str_token);
                    }
                }
            }
        ...
        Err(AliasSyntaxError::UnknownCharacter(
            self.curr_char(),
            self.kind,
            self.line,
            self.pos,
    ...
    fn test_deromanisation_simple() {
        ...
        let expected_result = vec![
            AliasToken::new(
                AliasTokenKind::String,
                "sh".to_owned(),
                AliasPosition::new(AliasKind::Deromaniser, 0, 0, 2),
            ),
            AliasToken::new(
                AliasTokenKind::GreaterThan,
                ">".to_owned(),
                AliasPosition::new(AliasKind::Deromaniser, 0, 3, 4),
            ),
            AliasToken::new(
                AliasTokenKind::Cardinal,
                "ʃ".to_owned(),
                AliasPosition::new(AliasKind::Deromaniser, 0, 5, 6),
            ),
            AliasToken::new(
                AliasTokenKind::Eol,
                String::new(),
                AliasPosition::new(AliasKind::Deromaniser, 0, 6, 7),
            ),
        ];

        let result = AliasLexer::new(
            AliasKind::Deromaniser,
            &test_input.chars().collect::<Vec<_>>(),
            0,
        )
        .get_line()
        .unwrap();
    ...
    fn test_deromanisation_stress() {
        ...
        let expected_result = vec![
            AliasToken::new(
                AliasTokenKind::String,
                "á".to_owned(),
                AliasPosition::new(AliasKind::Deromaniser, 0, 0, 1),
            ),
            AliasToken::new(
                AliasTokenKind::GreaterThan,
                ">".to_owned(),
                AliasPosition::new(AliasKind::Deromaniser, 0, 2, 3),
            ),
            AliasToken::new(
                AliasTokenKind::Cardinal,
                "a".to_owned(),
                AliasPosition::new(AliasKind::Deromaniser, 0, 4, 5),
            ),
            AliasToken::new(
                AliasTokenKind::Colon,
                ":".to_owned(),
                AliasPosition::new(AliasKind::Deromaniser, 0, 5, 6),
            ),
            AliasToken::new(
                AliasTokenKind::LeftSquare,
                "[".to_owned(),
                AliasPosition::new(AliasKind::Deromaniser, 0, 6, 7),
            ),
            AliasToken::new(
                AliasTokenKind::Feature(Supr(Stress)),
                "+".to_owned(),
                AliasPosition::new(AliasKind::Deromaniser, 0, 7, 11),
            ),
            AliasToken::new(
                AliasTokenKind::RightSquare,
                "]".to_owned(),
                AliasPosition::new(AliasKind::Deromaniser, 0, 11, 12),
            ),
            AliasToken::new(
                AliasTokenKind::Eol,
                String::new(),
                AliasPosition::new(AliasKind::Deromaniser, 0, 12, 13),
            ),
        ];
    ...
    fn test_romanisation_simple() {
        ...
        let expected_result = vec![
            ...
            AliasToken::new(
                AliasTokenKind::GreaterThan,
                ">".to_owned(),
                AliasPosition::new(AliasKind::Romaniser, 0, 2, 3),
            ),
            AliasToken::new(
                AliasTokenKind::String,
                "sh".to_owned(),
            ...
            AliasToken::new(
                AliasTokenKind::Eol,
                String::new(),
                AliasPosition::new(AliasKind::Romaniser, 0, 6, 7),
            ),
        ];
    ...
    fn test_romanisation_length() {
        ...
        let expected_result = vec![
            ...
            AliasToken::new(
                AliasTokenKind::Colon,
                ":".to_owned(),
                AliasPosition::new(AliasKind::Romaniser, 0, 1, 2),
            ),
            AliasToken::new(
                AliasTokenKind::LeftSquare,
                "[".to_owned(),
            ...
            AliasToken::new(
                AliasTokenKind::Feature(Supr(Long)),
                "-".to_owned(),
                AliasPosition::new(AliasKind::Romaniser, 0, 3, 8),
            ),
            AliasToken::new(
                AliasTokenKind::RightSquare,
                "]".to_owned(),
                AliasPosition::new(AliasKind::Romaniser, 0, 8, 9),
            ),
            AliasToken::new(
                AliasTokenKind::GreaterThan,
                ">".to_owned(),
                AliasPosition::new(AliasKind::Romaniser, 0, 10, 11),
            ),
            AliasToken::new(
                AliasTokenKind::String,
                "sh".to_owned(),
                AliasPosition::new(AliasKind::Romaniser, 0, 12, 14),
            ),
            AliasToken::new(
                AliasTokenKind::Eol,
                String::new(),
                AliasPosition::new(AliasKind::Romaniser, 0, 14, 15),
            ),
        ];
        ...
        let expected_result = vec![
            ...
            AliasToken::new(
                AliasTokenKind::String,
                "ssh".to_owned(),
                AliasPosition::new(AliasKind::Romaniser, 0, 12, 15),
            ),
            AliasToken::new(
                AliasTokenKind::Eol,
                String::new(),
                AliasPosition::new(AliasKind::Romaniser, 0, 15, 16),
            ),
        ];
    ...
    fn test_romanisation_stress() {
        ...
        let expected_result = vec![
            ...
            AliasToken::new(
                AliasTokenKind::Feature(Supr(Stress)),
                "+".to_owned(),
                AliasPosition::new(AliasKind::Romaniser, 0, 3, 7),
            ),
            AliasToken::new(
                AliasTokenKind::RightSquare,
                "]".to_owned(),
                AliasPosition::new(AliasKind::Romaniser, 0, 7, 8),
            ),
            AliasToken::new(
                AliasTokenKind::GreaterThan,
                ">".to_owned(),
                AliasPosition::new(AliasKind::Romaniser, 0, 9, 10),
            ),
            AliasToken::new(
                AliasTokenKind::String,
                "á".to_owned(),
            ...
            AliasToken::new(
                AliasTokenKind::Eol,
                String::new(),
                AliasPosition::new(AliasKind::Romaniser, 0, 12, 13),
            ),
        ];
    ...
    fn test_romanisation_named_escape() {
        ...
        let expected_result = vec![
            ...
            AliasToken::new(
                AliasTokenKind::String,
                "a\u{0301}".to_owned(),
                AliasPosition::new(AliasKind::Romaniser, 0, 11, 20),
            ),
            AliasToken::new(
                AliasTokenKind::Eol,
                String::new(),
                AliasPosition::new(AliasKind::Romaniser, 0, 20, 21),
            ),
        ];
    ...
    fn test_romanisation_literal_escape() {
        ...
        let expected_result = vec![
            ...
            AliasToken::new(
                AliasTokenKind::String,
                "a$".to_owned(),
                AliasPosition::new(AliasKind::Romaniser, 0, 11, 14),
            ),
            AliasToken::new(
                AliasTokenKind::Eol,
                String::new(),
